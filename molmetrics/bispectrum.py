...
def bispectrum(positions: jnp.ndarray, lmax: int) -> jnp.ndarray:
    ...
    x = e3nn.sum(e3nn.s2_dirac(positions, lmax=lmax, p_val=1, p_arg=-1), axis=0)
...
def compute_bispectrum_for_local_environment(
    env: LocalEnvironment, lmax: int
) -> np.ndarray:
...
class BispectraSamples:
    ...
    def aggregate_by_local_environment(self) -> Dict[LocalEnvironment, np.ndarray]:
        ...
        for env, sample in self.samples.items():
            ...
            env_no_positions = LocalEnvironment(
                env.central_atom, tuple([Atom(atom.symbol) for atom in env.neighbors])
            )
