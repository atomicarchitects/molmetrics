...
class RDKitMolecules(Molecules):
    ...
    def keep_valid(self) -> "RDKitMolecules":
        """Filters out invalid molecules."""
        valid_molecules = validity.get_all_valid_molecules(self)
        return RDKitMolecules(valid_molecules)

    def keep_unique(self) -> "RDKitMolecules":
        """Filters out duplicate molecules."""
        unique_molecules = uniqueness.get_all_unique_molecules(self)
        return RDKitMolecules(unique_molecules)
    ...
    def keep_non_identical(self, other: "RDKitMolecules") -> "RDKitMolecules":
        ...
        return RDKitMolecules(
            [mol for mol, smiles in zip(self, unique_smiles) if smiles not in other_smiles]
        )
